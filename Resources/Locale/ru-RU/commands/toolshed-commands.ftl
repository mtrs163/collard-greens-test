command-description-visualize = Берёт входной лист сущностей и помещает его в окно интерфейса для лёгкого поиска.
command-description-runverbas = Запускает действие через входные сущности с указанным пользователем.
command-description-acmd-perms = Возвращает требование к правам для приведённой команды, если таковые имеются.
